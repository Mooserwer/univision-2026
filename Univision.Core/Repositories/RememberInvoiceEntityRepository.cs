...
{
  public class RememberInvoiceEntityRepository : BaseRepository
  {
    public RememberInvoiceEntityRepository(UnivisionContext db) : base(db)
    {

    }
    public RememberInvoiceEntityRepository()
    {

    }

    public async Task<InvoiceInsertResult> InsertInvoiceWithCleanupAsync(invoice_new entity)
    {
      try
      {
        using (var tran = db.Database.BeginTransaction())
        {
          try
          {
            ...
            var existingInvoice = await db.invoice_news
            ...
            if (existingInvoice != null)
            {
              ...
              {
                return new InvoiceInsertResult(-2, "이미 승인 완료된 인보이스는 수정하거나 재등록할 수 없습니다.", null);
              }
            ...
            db.invoice_news.Add(entity);
            await db.SaveChangesAsync();

            tran.Commit();

            return new InvoiceInsertResult(1, "Success", entity);
    ...
    public async Task<InvoiceDeletableResult> CheckDeletableAsync(long r_invoice_id)
    {
      ...
      var invoice = await db.invoice_news
    ...
    {
      // 1. 데이터 존재 여부 확인 (is_deleted = 0 인 것 중)
      var root_invoice = await db.invoice_news
          .FirstOrDefaultAsync(x => x.r_invoice_id == r_invoice_id && x.is_deleted == 0);
      ...
      var canceled_invoice = await db.invoice_news
