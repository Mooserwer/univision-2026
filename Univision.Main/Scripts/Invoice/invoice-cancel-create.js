...
function setPiiSeq() {
  ...
  $('#billing_money').prop("readonly", "readonly");
  if ($pii.val() != undefined && $pii.val() != "")
    setPiiSales($pii.val());
}
...
function CalcRetire() {
  ...
  var work_day = datediffDay(join_dt, retire_dt);
  ...
  calc_str = "<h6 class='tx-success pd-t-5'>" + calc_str + "</h6>";
...
function TotalFeeChange() {
  ...
  if ($("#share_money_type:checked").val() != "1") {
    ...
    $("input[name='share_amt']").each(function () {
      var camt = parseInt(uncamma($(this).val()));
