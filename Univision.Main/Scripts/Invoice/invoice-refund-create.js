...
function chkStep4() {
  if ($("#frmInvoiceStep4").valid()) {
    $(".part_show_inv_step4_1").show();
    $("#btn_chk_step4").hide();
    setPiiSeq();
    $("#frmInvoiceStep4_1").valid()
    return true;
  } else {
    $("#btn_chk_step4").show();
...
//#region 후보자 선택
function setPiiSeq() {
  var $pii = GetPiiSeqObj();
  if ($pii.length > 0 && ($pii.data("invoice_type") == 0 || $pii.data("invoice_type") == 2)) {
    $('.part_show_pre_pii_seq').show();
    $('#ref_candidate_name').val($pii.data("candidate_name"));
    $('#ref_ann_income').val(comma($pii.data("ann_income")));
    $('#ref_billing_amt').val(comma($pii.data("billing_amt")));
    $('#ref_bill_currency_cd').text($pii.data("bill_currency_cd"));
    $('#bill_currency_cd').val($pii.data("bill_currency_cd"))
    //$('#ref_base_amt').val($pii.data("bill_currency_cd"));
    $('#ref_expire_guarantee').datepicker("update", $pii.data("expire_dt"));
    $('#ref_join_dt').val($pii.data("join_dt"));
    $('#vat_type_' + $pii.data("vat_type")).prop("checked", true);
    SetConstructDebut();
  } else {
    $('.part_show_pre_pii_seq').hide();
    $('#billing_money').val(comma($pii.data("billing_amt") * -1));
    $('#vat_type_' + $pii.data("vat_type")).prop("checked", true);
    $('#bill_currency_cd').val($pii.data("bill_currency_cd"));
    $('#billing_money').change();
  }
  setPiiSales($pii.val());
}

function setPiiSales(pii_seq) {
  ajxCall({
    url: window.url_invoice_share_list,
    data: {
      pii_seq: pii_seq
    },
    type: 'POST',
    dataType: 'html',
    loading: true,
    loading_target: "#share_list",
    bef: function () {
    },
    suc: function (html) {
      $('#share_list').html(html);
      try {
        $('#share_list').find(".share_fee_input").change(TotalFeeChange);
        SetShareMoneyType();
        $('#billing_money').change();
      } catch {
...
function SetConstructDebut() {
  var is_conde_yn = $("input:radio[name='construct_debut_yn']:checked").val();
  if (is_conde_yn == "Y") {
    $("#ref_base_amt").prop("readonly", "");
  } else {
    $("#ref_base_amt").prop("readonly", "readonly");
  }
}
$("#ref_expire_guarantee, #construct_debut_y, #construct_debut_n, #ref_base_amt, #retire_dt").change(CalcRetire);
function CalcRetire() {
  var calc_str = "";
  var billing_amt = 0
  
  if ($('#ref_billing_amt').val() != "") {
    billing_amt = uncomma($('#ref_billing_amt').val());
  } else {
    calc_str = "<h6 class='tx-danger'>발행금액 정보가 없습니다.<br/>다음을 눌러 수동으로 빌링금액을 입력해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }
  calc_str = "(" + comma(billing_amt) + " - ";

  base_amt = 0
  if ($("input:radio[name='construct_debut_yn']:checked").val() == "Y") {
    var ref_base_amt = Number($("#ref_base_amt").val()); 
    base_amt = billing_amt * (ref_base_amt * 0.01);
    calc_str += "(" + comma(billing_amt) + " * " + ref_base_amt + "%)) * ";
  } else {
    calc_str += "(0)) - "
  }

  var expire_dt = $('#ref_expire_guarantee').val()
  var join_dt = $('#ref_join_dt').val()
  var retire_dt = $('#retire_dt').val();

  if (retire_dt == "") {
    calc_str = "<h6 class='tx-danger'>퇴사일자를 입력해주세요. 입력해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }

  if (join_dt == "") {
    calc_str = "<h6 class='tx-danger'>입사일자 정보가 없습니다.<br/>다음을 눌러 수동으로 빌링금액을 입력해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }

  if (expire_dt == "") {
    calc_str = "<h6 class='tx-danger'>보증일자 정보가 없습니다. 입력해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }

  var grt_day = datediffDay(join_dt, expire_dt);
  var work_day = datediffDay(join_dt, retire_dt) + 1;

  if (work_day <= 0) {
    calc_str = "<h6 class='tx-danger'>근무일은 마이너스 일 수 없습니다. 다시 입력해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }

  if (grt_day <= work_day) {
    calc_str = "<h6 class='tx-danger'>후보자의 보증기간이 만료되었습니다. 보증기간 및 퇴사일을 다시 확인해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }

  if (grt_day <= 0) {
    calc_str = "<h6 class='tx-danger'>보증일수가 0일 입니다. 다시 확인해주세요.</h6>";
    $('#refund_calc_screen').html(calc_str);
    return;
  }
  ...
  calc_str += "(" + grt_day + "(보증일수) - " + work_day + "(근무일) / " + grt_day + "(보증일수))"
  calc_str += " = " + comma(Math.round((billing_amt - base_amt) * ((grt_day - work_day) / grt_day)));
  calc_str = "<h6 class='calc_str tx-success pd-t-5'>" + calc_str + "</h6>";
  var refund_amt = Math.round((billing_amt - base_amt) * ((grt_day - work_day) / grt_day)) * -1;

  calc_str += "<button type='button' class='btn btn-primary' onclick='SetBillingMoney(" + refund_amt + ");'>환불금액 적용하기</button>";

  $('#refund_calc_screen').html(calc_str);
}
function SetBillingMoney(amt) {
  
  $("#billing_money").val(comma(amt));
  $("#billing_money").change();

  var refund_contents = "\n---------- 아래는 자동으로 추가되는 환불 관련 정보입니다 ----------";
  refund_contents += "\n-후보자 : " + $("#ref_candidate_name").val()
  refund_contents += "\n-입사일 : " + $("#ref_join_dt").val()
  refund_contents += "\n-퇴사일 : " + $("#retire_dt").val()
  refund_contents += "\n-연봉 : " + $("#ref_ann_income").val()
  refund_contents += "\n-용역비 : " + $("#ref_billing_amt").val() + " " + $("#ref_bill_currency_cd").text()
  refund_contents += "\n-보증일 : " + datediffDay($("#ref_join_dt").val(), $("#ref_expire_guarantee").val()) + " 일 (" + $("#ref_expire_guarantee").val() + ")"
  refund_contents += "\n-근무일 : " + (datediffDay($("#ref_join_dt").val(), $("#retire_dt").val()) +1) + " 일"
  refund_contents += "\n-기초조사비 공제 : " + $("input:radio[name='construct_debut_yn']:checked").val()
  if ($("input:radio[name='construct_debut_yn']:checked").val() == "Y") {
    refund_contents += " (" + $("#ref_base_amt").val() +" %)"
  }
  refund_contents += "\n-계산식 : (용역비 - 기초조사비) * (보증일수 - 근무일수 / 보증일수)"
  refund_contents += "\n" + $(".calc_str").text();
  
  $("#invoice_remark").val(refund_contents);
  chkStep4_1();
}
...
var validator_step4_1 = $("#frmInvoiceStep4_1").validate($.extend({}, valid_option, invoice_step4_1));

function chkStep4_1() {

  if ($("#frmInvoiceStep4_1").valid()) {
    $(".part_show_inv_step5").show();
    $("#btn_chk_step4_1").hide();
    $("#frmInvoiceStep5").valid()
    return true;
  } else {

    $("#btn_chk_step4_1").show();
    return false;
  }
}
$("input:radio[name='construct_debut_yn']").on("change", SetConstructDebut);


$("input:radio[name='pre_pii_seq']").on("change", setPiiSeq);

$("#btn_chk_step4_1").click(chkStep4_1);
...
$('#candidate_source').change(function () {
  ...
  validator_step4_1.element('#candidate_source');
  validator_step4_1.element('#candidate_source_txt');
})

$('#candidate_position').change(function () {
  $('#candidate_position_txt').val($("#candidate_position option:selected").text());
  validator_step4_1.element('#candidate_position');
})

function fSetAddDay_Ref(addDay) {
  var join_dt_str = $('#ref_join_dt').val();
  if (join_dt_str == '') {
    join_dt_str = getDateStr(new Date());
  }
  //console.log(join_dt_str)
  if (addDay > 0) {
    $('#ref_expire_guarantee').datepicker("update", dateAdd(join_dt_str, addDay, "d"));
  }
}
...
function chkSubmit() {
  var chk1 = $("#frmInvoiceStep1").valid();
  var chk2 = $("#frmInvoiceStep4_1").valid();
  var chk3 = $("#frmInvoiceStep3").valid();
  var chk4 = $("#frmInvoiceStep4").valid();
  var chk5 = $("#frmInvoiceStep5").valid();
  var chk6 = $("#frmInvoiceStep6").valid();
...
function getCurrentData() {
  ...
  var currentData = {
    pii_seq: $("#pii_seq").val(),
    p_seq: $("#p_seq").val(),
    pre_pii: $("input[name=pre_pii_seq]:checked").val(),
    invoice_lang: lang,
    invoice_type: $("input[name=invoice_type]:checked").val(),
    billing_dt: $("#billing_dt").val(),
    tax_req_dt: $("#tax_req_dt").val(),
    //-------
    prc_seq: $pii.data("prc_seq"),
    join_dt: $pii.data("join_dt"),
    c_seq: $pii.data("c_seq"),
    candidate_name_kor: $("#ref_candidate_name").val(),
    candidate_name_eng: $("#ref_candidate_name").val(),
    expire_guarantee: $("#ref_expire_guarantee").val(),
    ...
    income_currency_cd: $pii.data("bill_currency_cd"),
